...
function NotificationCard({ notification, onOpen }) {
  ...
  return (
    <button
      ...
      className={`w-full text-left bg-white rounded-2xl p-5 border transition-all duration-300 group ${
        unread
          ? 'border-[#c17c74]/40 shadow-md hover:shadow-lg hover:-translate-y-0.5'
          : 'border-[#e5e2dd] shadow-xs hover:shadow-sm'
      }`}
      aria-label={unread ? `${meta.label}: ${notification.title} (unread)` : `${meta.label}: ${notification.title}`}
    >
      <div className="flex items-start gap-4">
        <div className={`w-11 h-11 rounded-2xl flex items-center justify-center text-[18px] shrink-0 ${unread ? 'bg-[#ffdad3]/60' : 'bg-[#f6f3ee]'}`} aria-hidden="true">
          {meta.icon}
        </div>
        <div className="min-w-0 flex-1">
          ...
          <p className={`mt-1 text-[14px] leading-snug ${unread ? 'font-semibold text-[#180f0a]' : 'text-[#4e4540]'}`}>
            {notification.title}
          </p>
          {notification.message && (
            <p className="mt-0.5 text-[13px] text-[#80756f] leading-relaxed">{notification.message}</p>
          )}
  ...
  return (
    <div ref={pageRef} className="w-full bg-[#fcf9f4] min-h-screen py-10 lg:py-16 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-[12px] font-semibold transition-all duration-300 ${
                  filter === f.key
                    ...
                    : 'text-[#4e4540] hover:text-[#180f0a] hover:bg-[#f6f3ee]'
                }`}
              >
                {f.label}
              </button>
        ...
        {/* Stream */}
        {loading ? (
          <div className="bg-white rounded-3xl border border-[#e5e2dd] p-14 text-center shadow-sm">
            <Loader2 className="w-6 h-6 text-[#964735] animate-spin mx-auto" />
            <p className="mt-3 text-[13px] text-[#80756f]">Gathering your updates…</p>
          </div>
        ) : error ? (
          <div className="bg-white rounded-3xl border border-[#e5e2dd] p-12 text-center shadow-sm space-y-4">
            <AlertCircle className="w-10 h-10 text-[#c17c74] mx-auto" />
            <p className="font-serif text-[20px] text-[#180f0a]">Something went wrong</p>
            <p className="text-[13px] text-[#80756f]">{error}</p>
            <button
              type="button"
              onClick={load}
          ...
          <div className="relative bg-white rounded-3xl p-12 text-center border border-[#e5e2dd] shadow-sm space-y-3 overflow-hidden">
            ...
            <p className="relative font-serif text-[22px] text-[#180f0a]">
              {filter === 'unread' ? 'All caught up' : 'Nothing here yet'}
            </p>
            <p className="relative text-[13px] text-[#80756f] max-w-sm mx-auto">
              {filter === 'unread'
            ...
            {filter === 'all' && (
              <div className="relative pt-2 flex flex-wrap justify-center gap-3">
                ...
                <Link to="/account" className="px-6 py-2.5 rounded-full border border-[#e5e2dd] bg-white text-[#180f0a] text-[12px] font-semibold hover:bg-[#f6f3ee] transition-colors">My Account</Link>
              </div>
            )}
          </div>
        ) : (
