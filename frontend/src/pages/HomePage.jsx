  ...
  return (
    <div className="w-full">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center min-h-[580px]">
            <div className="lg:col-span-7 flex flex-col items-start gap-4">
              ...
              <div data-hero-badge data-cursor-depth="0.3" className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#ebe8e3] text-[#4e4540] shadow-sm opacity-0">
              ...
              <div data-hero-headline data-cursor-depth="0.15" className="opacity-0">
                <h1 className="font-serif text-[36px] sm:text-[48px] md:text-[54px] lg:text-[62px] text-[#180f0a] font-normal tracking-tight leading-[1.08] max-w-2xl">
                  Handmade with love.{' '}
              ...
              <div data-hero-sub className="opacity-0">
                <p className="text-[15px] sm:text-[17px] text-[#4e4540] max-w-xl leading-relaxed">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 w-full">
                {[{ emoji: '🌿', bg: '#d8e7cd', label: 'Handmade with care' }, { emoji: '🌸', bg: '#ffdad3', label: 'Everlasting blooms' }, { emoji: '💌', bg: '#ebe8e3', label: 'Handwritten wax card' }].map((b, i) => (
                  <div key={b.label} data-hero-trust data-cursor-depth={0.2 + i * 0.05} className="flex items-center gap-2.5 p-3 rounded-2xl bg-white/80 border border-[#e5e2dd] shadow-xs hover:-translate-y-0.5 transition-transform duration-300 opacity-0">
            ...
            <div className="lg:col-span-5 relative h-[400px] sm:h-[480px] lg:h-[600px] w-full flex items-center justify-center">
              ...
              <div data-hero-image data-parallax="0.08" className="relative w-full h-[340px] sm:h-[420px] lg:h-[540px] rounded-3xl overflow-hidden shadow-2xl border border-[#e5e2dd] opacity-0">
              ...
              <div data-cursor-depth="0.4" className="absolute bottom-4 sm:bottom-6 left-3 right-3 sm:left-4 sm:right-4 p-3 rounded-2xl bg-white/90 backdrop-blur-md shadow-md flex items-center justify-between">
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-[#964735] animate-ping" /><span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-[#1c1c19]">Flora Alchemy Atelier</span></div>
                <span className="text-[11px] sm:text-[12px] text-[#4e4540] italic">Handcrafted in India</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ═══════════════════════════════════════════════════════════════
          ...
          ═══════════════════════════════════════════════════════════════ */}
      <section data-story-collections className="w-full py-16 lg:py-24 bg-[#fcf9f4]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-12">
            <div className="max-w-xl space-y-1">
              ...
              <h2 className="font-serif text-[28px] sm:text-[32px] lg:text-[40px] text-[#180f0a] tracking-tight font-normal">Curated Craft Collections</h2>
              <p className="text-[14px] sm:text-[15px] text-[#4e4540]">Explore our signature handmade creations sculpted one stem, fiber, and stitch at a time.</p>
            </div>
          ...
          <div data-stagger-grid data-reveal className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
            {[
              ...
              <Link key={card.title} to={card.to} className={`group relative rounded-3xl p-5 lg:p-6 transition-all duration-300 flex flex-col justify-between h-68 lg:h-72 overflow-hidden border border-[#e5e2dd] hover:shadow-xl hover:-translate-y-1 ${card.dark ? 'bg-[#2e241e] text-white border-[#180f0a]' : 'bg-[#f6f3ee] hover:bg-[#f0ede9]'}`}>
                ...
                <div className="relative z-10 space-y-1">
                  <h3 className={`font-serif text-[20px] lg:text-[22px] transition-colors ${card.dark ? 'text-white group-hover:text-[#ffdad3]' : 'text-[#180f0a] group-hover:text-[#964735]'}`}>{card.title}</h3>
                  <p className={`text-[13px] leading-relaxed ${card.dark ? 'text-[#d4c3ba]' : 'text-[#4e4540]'}`}>{card.desc}</p>
                  <div className={card.dark ? "pt-2 inline-flex items-center gap-1 text-[12px] font-bold text-[#ffdad3]" : "pt-2 inline-flex items-center gap-1 text-[12px] font-bold text-[#180f0a]"}><span>{card.cta}</span><ArrowRight className="w-3.5 h-3.5 transition-transform duration-200 group-hover:translate-x-1" /></div>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-14">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            <div className="lg:col-span-4 space-y-3 lg:sticky lg:top-28">
              ...
              <h2 className="font-serif text-[28px] sm:text-[32px] lg:text-[38px] text-[#180f0a] tracking-tight font-normal leading-tight">Shop by occasion.</h2>
              <p className="text-[14px] sm:text-[15px] text-[#4e4540] leading-relaxed">Start from the moment you&apos;re celebrating. Each occasion opens the handcrafted pieces that suit it.</p>
            ...
            <div data-reveal data-stagger-grid className="lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-3">
              {FEATURED_OCCASIONS.map((o) => (
                <Link key={o.id} to={`/shop?occasion=${o.id}`} className="group flex flex-col justify-between gap-3 p-4 lg:p-5 rounded-3xl bg-[#f6f3ee] border border-[#e5e2dd] hover:bg-[#f0ede9] hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 min-h-[100px] lg:min-h-[108px]">
                  <span className="text-xl lg:text-2xl" aria-hidden="true">{o.icon}</span>
                  <span className="flex items-center justify-between gap-2">
                    <span className="font-serif text-[16px] lg:text-[18px] text-[#180f0a] group-hover:text-[#964735] transition-colors">{o.label}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start pt-10 border-t border-[#e5e2dd]">
            <div className="lg:col-span-4 space-y-3 lg:sticky lg:top-28">
              ...
              <h2 className="font-serif text-[28px] sm:text-[32px] lg:text-[38px] text-[#180f0a] tracking-tight font-normal leading-tight">Shop by recipient.</h2>
              <p className="text-[14px] sm:text-[15px] text-[#4e4540] leading-relaxed">Choose who the gift is for and browse pieces our studio most often crafts for them.</p>
            </div>
            <div data-stagger-grid className="lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-3">
              {FEATURED_RECIPIENTS.map((r) => (
                <Link key={r.id} to={`/shop?recipient=${r.id}`} className="group flex flex-col justify-between gap-3 p-4 lg:p-5 rounded-3xl bg-white border border-[#e5e2dd] hover:border-[#964735] hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 min-h-[100px] lg:min-h-[108px]">
                  <span className="text-xl lg:text-2xl" aria-hidden="true">{r.icon}</span>
                  <span className="flex items-center justify-between gap-2">
                    <span className="font-serif text-[16px] lg:text-[18px] text-[#180f0a] group-hover:text-[#964735] transition-colors">{r.label}</span>
                    <ArrowRight className="w-4 h-4 text-[#80756f] group-hover:text-[#964735] transition-all duration-200 group-hover:translate-x-0.5" aria-hidden="true" />
                  </span>
                </Link>
              ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12">
            <div>
              ...
              <h2 className="font-serif text-[28px] sm:text-[32px] lg:text-[40px] text-[#180f0a] tracking-tight font-normal">Beloved Creations</h2>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              {[{ key: 'all', label: 'All Keepsakes' }, { key: 'bouquets', label: 'Sculpted Bouquets' }, { key: 'cards', label: 'Botanical Cards' }, { key: 'hampers', label: 'Gift Boxes' }].map((f) => (
                <button key={f.key} onClick={() => setBestsellerFilter(f.key)} className={`px-3.5 lg:px-4 py-1.5 rounded-full text-[12px] font-semibold transition-all duration-200 ${bestsellerFilter === f.key ? 'bg-[#180f0a] text-white shadow-sm' : 'bg-white text-[#4e4540] hover:text-[#180f0a] border border-[#e5e2dd] hover:border-[#180f0a]'}`}>{f.label}</button>
              ))}
            </div>
          </div>
      ...
      <section className="w-full py-16 lg:py-24 bg-[#fcf9f4]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div data-story-climb className="text-center max-w-2xl mx-auto mb-16 space-y-2">
            ...
            <h2 className="font-serif text-[28px] sm:text-[36px] lg:text-[44px] text-[#180f0a] tracking-tight font-normal">Create something that is uniquely theirs.</h2>
            <p className="text-[14px] sm:text-[15px] text-[#4e4540] leading-relaxed">Step into our craft studio. We personalize your heartfelt vision from individual sculpted petals to customized wax-stamped gift tags.</p>
          </div>
          <div data-reveal data-stagger-grid className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6 mb-16">
            {[
              ...
              <div key={s.num} className="p-5 lg:p-6 rounded-3xl bg-[#f6f3ee] flex flex-col justify-between space-y-4 border border-[#e5e2dd] hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                ...
                <div><h3 className="font-serif text-[18px] lg:text-[20px] text-[#180f0a] mb-1 font-medium">{s.title}</h3><p className="text-[13px] text-[#4e4540] leading-relaxed">{s.desc}</p></div>
                <span className="text-[11px] font-bold uppercase tracking-wider" style={{ color: s.tc }}>{s.tag}</span>
              </div>
            ))}
          </div>
          <div data-fade className="rounded-3xl bg-[#f6f3ee] p-5 lg:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center border border-[#e5e2dd] shadow-sm">
            <div className="lg:col-span-7 space-y-4">
              <span className="text-[11px] uppercase font-bold tracking-widest text-[#964735]">Live Atelier Previewer</span>
              <h3 className="font-serif text-[22px] sm:text-[26px] lg:text-[32px] text-[#180f0a] tracking-tight font-normal">Try Our Instant Gift Note & Wax Seal Customizer</h3>
              <p className="text-[13px] sm:text-[14px] text-[#4e4540]">Type your message below and watch it render live on our simulated deckled cotton card with your choice of wax seal.</p>
              <div className="space-y-3 max-w-lg">
                <div><label className="block text-[11px] uppercase font-bold text-[#4e4540] mb-1">Envelope Addressee / Recipient</label><input type="text" value={monogramText} onChange={(e) => setMonogramText(e.target.value)} className="w-full px-4 py-2.5 rounded-full bg-white text-[14px] text-[#1c1c19] border border-[#e5e2dd] focus:outline-none focus:ring-1 focus:ring-[#180f0a] transition-shadow" /></div>
            ...
            <div className="lg:col-span-5 flex justify-center">
              <div className="relative w-full max-w-sm p-5 lg:p-6 rounded-2xl bg-[#faf7f2] shadow-xl border border-[#e5e2dd] rotate-1 hover:rotate-0 transition-all duration-500 hover:shadow-2xl">
                ...
                <div className="space-y-3">
                  <div className="border-b border-[#e5e2dd] pb-2"><span className="text-[10px] uppercase font-bold tracking-widest text-[#964735]">Deckled Cotton Card</span><p className="font-serif text-[16px] lg:text-[18px] text-[#180f0a] italic">{monogramText || 'For Someone Special'}</p></div>
                  <p className="font-serif text-[14px] lg:text-[16px] text-[#1c1c19] leading-relaxed italic pt-1">&ldquo;{messageText || 'Thinking of you with fond botanical thoughts.'}&rdquo;</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div data-reveal className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
            <div className="lg:col-span-6 relative">
              <div data-story-img-reveal data-parallax="0.06" className="relative rounded-3xl overflow-hidden shadow-xl aspect-[4/3] border border-[#e5e2dd]">
              ...
              <div className="absolute -bottom-4 right-2 sm:right-6 p-3 lg:p-4 rounded-2xl bg-white shadow-lg border border-[#e5e2dd] flex items-center gap-3 hover:-translate-y-0.5 transition-transform duration-300">
                <span className="text-xl lg:text-2xl">🌱</span>
                <div><p className="text-[11px] font-bold uppercase text-[#964735]">Genuine Craft</p><p className="text-[12px] lg:text-[13px] font-semibold text-[#180f0a]">Handmade in Small Batches</p></div>
              </div>
            </div>
            <div className="lg:col-span-6 space-y-4">
              <span className="text-[11px] font-bold uppercase tracking-widest text-[#964735]">Our Studio Atelier</span>
              <h2 className="font-serif text-[28px] sm:text-[32px] lg:text-[40px] text-[#180f0a] tracking-tight leading-tight font-normal">Crafting flowers designed to <span className="italic font-light text-[#964735]">endure</span>.</h2>
              <p className="text-[14px] sm:text-[15px] text-[#4e4540] leading-relaxed">Flora Alchemy began with a quiet desire for gifts that outlive fleeting moments. Every flower petal is individually shaped from high-density velvet chenille wire, bound with unbleached cotton threads, and accompanied by hand-deckled cards.</p>
              <p className="text-[14px] sm:text-[15px] text-[#4e4540] leading-relaxed">When you hold our creations, you feel the soft plush texture of velvet wire, the organic deckle of rag paper, and the personal touch of wax seals stamped by hand.</p>
              <div data-stagger-grid className="grid grid-cols-3 gap-3 pt-3">
                {[{ l: 'Handmade', s: 'Petal-by-petal' }, { l: 'Personalized', s: 'With wax seals' }, { l: 'Made to Order', s: 'Tailored gifting' }].map((i) => (
                  <div key={i.l} className="p-3 lg:p-3.5 rounded-2xl bg-[#f6f3ee] text-center border border-[#e5e2dd] hover:-translate-y-0.5 hover:shadow-md transition-all duration-300"><p className="font-serif text-[16px] lg:text-[18px] text-[#180f0a] font-medium">{i.l}</p><p className="text-[10px] lg:text-[11px] text-[#80756f]">{i.s}</p></div>
                ))}
              </div>
            </div>
          </div>
      ...
      <section className="w-full py-16 bg-[#f6f3ee] border-t border-[#e5e2dd]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div data-fade className="text-center max-w-xl mx-auto mb-12 space-y-1"><span className="text-[11px] font-bold uppercase tracking-widest text-[#964735]">The Atelier Creed</span><h2 className="font-serif text-[28px] sm:text-[32px] lg:text-[38px] text-[#180f0a] tracking-tight font-normal">Four Pillars of Every Creation</h2></div>
          <div data-reveal data-stagger-grid className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
            {[
              ...
              <div key={p.n} className="p-5 lg:p-6 rounded-3xl bg-white shadow-sm flex flex-col justify-between border border-[#e5e2dd] hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"><span className="text-[11px] font-bold uppercase tracking-wider text-[#964735]">{p.n}</span><h3 className="font-serif text-[18px] lg:text-[20px] text-[#180f0a] my-2 font-medium">{p.t}</h3><p className="text-[13px] text-[#4e4540] leading-relaxed">{p.d}</p></div>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="w-full py-16 lg:py-24 bg-[#fcf9f4]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div data-story-climb className="mb-12"><span className="text-[11px] font-bold uppercase tracking-widest text-[#964735]">Gifting Inscriptions</span><h2 className="font-serif text-[28px] sm:text-[32px] lg:text-[40px] text-[#180f0a] tracking-tight font-normal">Card Messages & Dedicated Sentiments</h2></div>
          <div data-reveal data-stagger-grid className="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-6">
            {[
              ...
              <div key={c.l} className="p-6 lg:p-7 rounded-3xl bg-[#f6f3ee] flex flex-col justify-between space-y-4 border border-[#e5e2dd] hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                <span className="text-[11px] uppercase font-bold text-[#964735]">{c.l}</span>
                <p className="font-serif text-[16px] lg:text-[18px] text-[#180f0a] italic leading-relaxed">{c.q}</p>
      ...
      <section className="w-full py-16 lg:py-20 bg-[#ebe8e3] text-center">
        <div data-story-climb className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
          ...
          <h2 className="font-serif text-[30px] sm:text-[36px] lg:text-[46px] text-[#180f0a] tracking-tight font-normal leading-[1.1]">Make someone&apos;s ordinary day feel <span className="italic font-light text-[#964735]">extraordinary</span>.</h2>
          <p className="text-[14px] sm:text-[16px] text-[#4e4540] max-w-xl mx-auto leading-relaxed">Whether it&apos;s a silent gesture of gratitude, an anniversary milestone, or just a little something to make them smile today.</p>
          <div className="pt-4 flex flex-wrap items-center justify-center gap-3">
            ...
            <span data-magnetic><Link to="/custom-gifts" className="px-7 lg:px-8 py-3.5 rounded-full bg-white text-[#180f0a] hover:bg-[#f0ede9] transition-all text-[13px] font-semibold shadow-sm border border-[#e5e2dd] hover:shadow-md hover:-translate-y-0.5">Custom Gift Studio</Link></span>
          </div>
