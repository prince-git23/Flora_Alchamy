      ...
      <div className="max-w-7xl mx-auto space-y-6 pb-12">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[14px] text-[#4e4540] mt-1">Curated product collections and seasonal groupings</p>
          </div>
          <button
            type="button"
        ...
        {/* Create panel */}
        {showCreate && (
          <form onSubmit={handleCreate} className="bg-white rounded-xl border border-[#e5e2dd] p-6 shadow-xs space-y-4" noValidate>
            <h2 className="font-serif text-lg text-[#180f0a] font-medium">Create Collection</h2>
            {createError && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor="col-vis" className="block text-[12px] font-semibold text-[#4e4540] mb-1.5">Visibility</label>
            ...
            <div className="flex items-center justify-end gap-2">
              <button type="button" onClick={() => { setShowCreate(false); setCreateError(''); }}
                className="px-4 py-2 rounded-full border border-[#d1c4bd] text-[12px] font-semibold text-[#4e4540] hover:bg-[#f6f3ee] transition">
                Cancel
              </button>
        ...
        {filtered.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((col) => (
              <div key={col.id} className="relative bg-white rounded-xl border border-[#e5e2dd] shadow-xs hover:shadow-md hover:border-[#d1c4bd] transition-all overflow-hidden group">
                  ...
                  <>
                    <Link to={`/admin/collections/${col.id}`} className="block">
                      ...
                      <div className="p-5 space-y-2">
                        ...
                        <p className="text-[13px] text-[#4e4540] line-clamp-2">{col.description || 'No description.'}</p>
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 p-1.5 rounded-full bg-white/95 shadow text-[#ba1a1a] hover:bg-[#ffdad6] transition-opacity"
