    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-8 pb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl p-5 shadow-[0_4px_20px_-2px_rgba(46,36,30,0.04)] border border-[#e5e2dd]">
            ...
            <div className="mt-2">
              <span className="font-serif text-3xl font-medium text-[#180f0a]">{users.length}</span>
            </div>
            <p className="mt-1 text-[13px] text-[#4e4540]">{adminCount} admins · {handlerCount} handlers</p>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-[0_4px_20px_-2px_rgba(46,36,30,0.04)] border border-[#e5e2dd]">
            <div className="flex items-center justify-between text-[#80756f]">
              <span className="text-[11px] font-bold uppercase tracking-wider">Administrators</span>
              <span className="material-symbols-outlined text-[#180f0a] text-[20px]">admin_panel_settings</span>
            </div>
            <div className="mt-2">
              <span className="font-serif text-3xl font-medium text-[#180f0a]">{adminCount}</span>
            </div>
            <p className="mt-1 text-[13px] text-[#4e4540]">Full system access</p>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-[0_4px_20px_-2px_rgba(46,36,30,0.04)] border border-[#e5e2dd]">
            <div className="flex items-center justify-between text-[#80756f]">
            ...
            <div className="mt-2">
              <span className="font-serif text-3xl font-medium text-[#180f0a]">{handlerCount}</span>
            </div>
            <p className="mt-1 text-[13px] text-[#4e4540]">Order & catalog management</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_-2px_rgba(46,36,30,0.04)] border border-[#e5e2dd] p-6 sm:p-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6">
            <div>
              <h2 className="font-serif text-2xl text-[#180f0a] font-medium">Operator Directory</h2>
              <p className="text-[13px] text-[#4e4540] mt-0.5">All operators stored in the database.</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between py-3 px-4 bg-[#f6f3ee] rounded-xl mb-6">
            <div className="relative flex-1 max-w-md">
              <span className="material-symbols-outlined absolute left-3 top-2.5 text-[18px] text-[#80756f]">search</span>
              <input
                type="text"
                value={searchQuery}
          ...
          {loaded && !loadError && (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-[13px]">
                ...
                <tbody className="divide-y divide-[#f0ede9]">
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((u) => (
                      <tr key={u.id} className="hover:bg-[#f6f3ee]/50 transition-colors">
                        <td className="py-3.5 px-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[13px] font-bold shadow-xs ${u.role === 'ADMINISTRATOR' ? 'bg-[#180f0a] text-white' : 'bg-[#ebe8e3] text-[#1c1c19]'}`}>
                              {u.initials}
                            </div>
                            <div>
                              <p className="font-semibold text-[#180f0a] leading-tight">{u.name}</p>
                              {u.isFixture && <span className="text-[10px] text-[#80756f]">Seed account</span>}
                            </div>
                          </div>
                        </td>
                        <td className="py-3.5 px-4">
              ...
              <div className="pt-4 mt-2 border-t border-[#f0ede9] text-[13px] text-[#80756f]">
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_-2px_rgba(46,36,30,0.04)] border border-[#e5e2dd] p-6 sm:p-8">
          <h2 className="font-serif text-2xl text-[#180f0a] font-medium mb-4">Role Permission Matrix</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[13px]">
              <thead>
                <tr className="text-[#80756f] text-[11px] font-bold uppercase tracking-wider bg-[#f6f3ee]/60">
              ...
              <tbody className="divide-y divide-[#f0ede9]">
                {[
                  ...
                  <tr key={module} className="hover:bg-[#f6f3ee]/40 transition-colors">
                    <td className="py-3 px-4 font-semibold text-[#180f0a]">{module}</td>
                    <td className="py-3 px-4 text-[#1d2918]">{admin}</td>
                    <td className="py-3 px-4 text-[#4e4540]">{handler}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-[#e5e2dd] animate-fade-in space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setShowAddModal(false)} className="p-1 rounded-lg text-[#80756f] hover:bg-[#f0ede9]">
                  <span className="material-symbols-outlined text-[20px]">close</span>
                </button>
              </div>
              <form onSubmit={handleAddHandler} className="space-y-4 text-[13px]">
                <div>
                  <label className="block text-[11px] font-bold uppercase text-[#80756f] mb-1">Full Name</label>
                  <input
                    type="text"
                    required
                    value={newUserName}
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase text-[#80756f] mb-1">Email</label>
                  <input
                    type="email"
                    required
                    value={newUserEmail}
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase text-[#80756f] mb-1">Role</label>
                  <select
                    value={newUserRole}
                ...
                <div className="pt-3 flex items-center justify-end gap-2">
                  <button type="button" onClick={() => setShowAddModal(false)} className="px-4 py-2 rounded-full text-[#4e4540] hover:bg-[#f6f3ee] font-semibold">Cancel</button>
                  <button
                    type="submit"
                    disabled={saving}
